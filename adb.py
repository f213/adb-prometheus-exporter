...
class Device:
    ...
    def list_all(cls, populate: bool = False) -> list["Device"]:
        ...
        for line in result.stdout.decode().split('\n'):
            device = line.split('\t')
            if len(device) == 2:
                if 'unauthorized' not in device[1]:
                    device = cls(serial=device[0], is_authorized=True)
                    if populate:
                        device.populate()
                    devices.append(device)
                else:
                    devices.append(cls(serial=device[0]))
    ...
    def fetch_temperature(self) -> None:
        result = subprocess.run(['adb', '-s', self.serial, 'shell', 'dumpsys', 'thermalservice' ], capture_output=True)
